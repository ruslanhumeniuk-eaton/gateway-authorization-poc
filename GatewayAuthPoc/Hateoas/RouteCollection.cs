...
internal class RouteCollection : IRouteCollection
{
    ...
    private IEnumerable<Route> GetRoutes(IEnumerable<OcelotRouteConfiguration> ocelotRoutes) => ocelotRoutes.Select(GetRoute).Where(r => !string.IsNullOrWhiteSpace(r.Name));
    ...
    private Route GetRoute(OcelotRouteConfiguration ocelotRouteConfiguration)
    {
        ...
        {
            permissions.AddRange(ocelotRouteConfiguration.PermissionKeys.Select(x => new Permission(x, OrganizationIdRouteKey)));
        }
